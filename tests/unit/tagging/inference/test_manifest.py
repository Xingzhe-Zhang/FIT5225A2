...
def write_manifest(root: Path, *, version: str, classifier: bytes) -> Path:
    ...
    labels = b"dingo\nwombat\n"
    (root / "detector.bin").write_bytes(detector)
    (root / f"classifier-{version}.bin").write_bytes(classifier)
    (root / "labels.txt").write_bytes(labels)
    manifest = {
        "schema_version": "1.0",
        "model_version": version,
        "detector": {"uri": str(root / "detector.bin"), "sha256": sha256(detector)},
        "classifier": {
            "uri": str(root / f"classifier-{version}.bin"),
